...
static int32_t field() { return -2; }
static int32_t advance(uint32_t bytes) {
  assert(bytes <= INT32_MAX && "advance bytes must fit in int32");
  return static_cast<int32_t>(bytes);
}
static int32_t skip(size_t count) {
  assert(count <= INT32_MAX - 3 && "skip count must fit in int32");
  return static_cast<int32_t>(-3 - count);
}
// The following parsing routine is currently no used.
// struct End {};
// struct Variant {};
// struct Field {};
// struct Advance {
