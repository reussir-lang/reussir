...
nullableDispatch ::
    V.TypedValue -> IR.Block -> IR.Block -> Maybe V.TypedValue -> IR.Instr
...
comesAfter needle1 needle2 haystack =
    isInfixOf needle1 haystack
        ...
        && case ( T.breakOn (T.pack needle1) (T.pack haystack)
                , T.breakOn (T.pack needle2) (T.pack haystack)
                ) of
    ...
    testGroup
        ...
        [ testGroup
            ...
            [ testCase "ICall Codegen" $ do
                ...
                assertBool "Should contain args %1 and %2" $
                    "%1" `isInfixOf` resultStr && "%2" `isInfixOf` resultStr
        ...
        , testGroup
            ...
            [ testCase "FCall Codegen without result" $ do
                ...
                assertBool "Should not contain result assignment" $
                    not (" = func.call" `isInfixOf` resultStr)
            , testCase "FCall Codegen with result" $ do
                ...
                assertBool "Result should come before func.call" $
                    comesAfter "%2 = " "func.call" resultStr
            , testCase "FCall Codegen with multiple args" $ do
                ...
                assertBool "Should contain result %3" $ "%3 = " `isInfixOf` resultStr
                assertBool "Should contain test_func" $ "test_func" `isInfixOf` resultStr
                assertBool "Should contain both args" $
                    "%1" `isInfixOf` resultStr && "%2" `isInfixOf` resultStr
                assertBool "Should contain i32 and i64 types" $
                    "i32" `isInfixOf` resultStr && "i64" `isInfixOf` resultStr
                assertBool "Should contain return type" $ " -> " `isInfixOf` resultStr
            ]
        , testGroup
            "Panic"
            [ testCase "Panic Codegen" $ do
                ...
                let resultStr = T.unpack result
                assertBool "Should contain reussir.panic" $
                    "reussir.panic" `isInfixOf` resultStr
                assertBool "Should contain error message" $
                    "error message" `isInfixOf` resultStr
        ...
        , testGroup
            ...
            , testCase "Return Codegen with value" $ do
                ...
                assertBool "Should contain i32 type" $ "i32" `isInfixOf` resultStr
                assertBool "Value should come after return" $
                    comesAfter "func.return" "%1" resultStr
            ]
        , testGroup
            "NullableCheck"
            [ testCase "NullableCheck Codegen" $ do
                ...
                let resultStr = T.unpack result
                assertBool "Should contain reussir.nullable.check" $
                    "reussir.nullable.check" `isInfixOf` resultStr
                assertBool "Should contain result %2" $ "%2 = " `isInfixOf` resultStr
                assertBool "Should contain nullable type" $
                    "nullable" `isInfixOf` resultStr || "!reussir.nullable" `isInfixOf` resultStr
                assertBool "Should contain input %1" $ "%1" `isInfixOf` resultStr
                assertBool "Should contain boolean type or i1" $
                    "i1" `isInfixOf` resultStr || "bool" `isInfixOf` resultStr
                assertBool "Result should come before operation" $
                    comesAfter "%2 = " "reussir.nullable.check" resultStr
            ]
        , testGroup
            "NullableCreate"
            [ testCase "NullableCreate Codegen without value" $ do
                ...
                let resultStr = T.unpack result
                assertBool "Should contain reussir.nullable.create" $
                    "reussir.nullable.create" `isInfixOf` resultStr
                assertBool "Should contain result %1" $ "%1 = " `isInfixOf` resultStr
                assertBool "Should contain nullable type" $
                    "nullable" `isInfixOf` resultStr || "!reussir.nullable" `isInfixOf` resultStr
                assertBool "Should contain type annotation" $ " : " `isInfixOf` resultStr
                assertBool "Should not contain value argument" $
            ...
            , testCase "NullableCreate Codegen with value" $ do
                ...
                assertBool "Should contain nullable type" $
                    "nullable" `isInfixOf` resultStr || "!reussir.nullable" `isInfixOf` resultStr
                assertBool "Should contain rc type" $
                    "reussir.rc" `isInfixOf` resultStr || "rc" `isInfixOf` resultStr
                assertBool "Value should come before nullable type" $
                    comesAfter "%2" "nullable" resultStr
                        || comesAfter "%2" "!reussir.nullable" resultStr
            ]
        , testGroup
            "NullableDispatch"
            [ testCase "NullableDispatch Codegen without result" $ do
                ...
                let resultStr = T.unpack result
                assertBool "Should contain reussir.nullable.dispatch" $
                    "reussir.nullable.dispatch" `isInfixOf` resultStr
                assertBool "Should contain input %1" $ "%1" `isInfixOf` resultStr
                assertBool "Should contain nullable type" $
                    "nullable" `isInfixOf` resultStr || "!reussir.nullable" `isInfixOf` resultStr
                ...
                assertBool "nonnull should come before null" $
                    comesAfter "nonnull ->" "null ->" resultStr
            ...
            , testCase "NullableDispatch Codegen with instructions in blocks" $ do
                ...
                assertBool "Should contain reussir.nullable.dispatch" $
                    "reussir.nullable.dispatch" `isInfixOf` resultStr
                assertBool "Should contain func.return in nonnull block" $
                    "func.return" `isInfixOf` resultStr
        ...
        , testGroup
            ...
            [ testCase "WithLoc Codegen with FileLineColRange" $ do
                ...
                assertBool "Location should come after instruction" $
                    comesAfter "func.return" "loc" resultStr
                assertBool "Should contain trailing locs marker" $
                    "========= trailing locs =========" `isInfixOf` resultStr
                assertBool "Should contain outline location definition" $
                    "#loc0 = " `isInfixOf` resultStr
                assertBool "Outline location should come after marker" $
                    comesAfter "========= trailing locs =========" "#loc0 = " resultStr
                assertBool "Should contain file location in outline" $
                    "test.hs" `isInfixOf` resultStr
                assertBool "Should contain line/column info in outline" $
                    "10:5" `isInfixOf` resultStr || "10:5 to 10:15" `isInfixOf` resultStr
            , testCase "WithLoc Codegen with UnknownLoc" $ do
                ...
                assertBool "Should contain #loc0" $ "#loc0" `isInfixOf` resultStr
                assertBool "Location should come after instruction" $
                    comesAfter "reussir.panic" "loc" resultStr
                assertBool "Should contain trailing locs marker" $
                    "========= trailing locs =========" `isInfixOf` resultStr
                assertBool "Should contain outline location definition" $
                    "#loc0 = " `isInfixOf` resultStr
                assertBool "Outline location should come after marker" $
                    comesAfter "========= trailing locs =========" "#loc0 = " resultStr
                assertBool "Should contain unknown location in outline" $
                    "?" `isInfixOf` resultStr || "loc(?)" `isInfixOf` resultStr
            ]
        , testGroup
            "RcInc"
            [ testCase "RcInc Codegen" $ do
                ...
                let resultStr = T.unpack result
                assertBool "Should contain reussir.rc.inc" $
                    "reussir.rc.inc" `isInfixOf` resultStr
                assertBool "Should contain input %1" $ "%1" `isInfixOf` resultStr
                assertBool "Should contain rc type" $
                    "reussir.rc" `isInfixOf` resultStr || "rc" `isInfixOf` resultStr
        ...
        , testGroup
            ...
            [ testCase "RcCreate Codegen without region" $ do
                ...
                let resultStr = T.unpack result
                assertBool "Should contain reussir.rc.create" $
                    "reussir.rc.create" `isInfixOf` resultStr
                assertBool "Should contain result %2" $ "%2 = " `isInfixOf` resultStr
                ...
                assertBool "Should contain rc type" $
                    "reussir.rc" `isInfixOf` resultStr || "rc" `isInfixOf` resultStr
                assertBool "Result should come before operation" $
                    comesAfter "%2 = " "reussir.rc.create" resultStr
                assertBool "Should not contain region parameter" $
                    not ("region(" `isInfixOf` resultStr)
            , testCase "RcCreate Codegen with region" $ do
                ...
                assertBool "Should contain region type" $
                    "reussir.region" `isInfixOf` resultStr || "region" `isInfixOf` resultStr
                assertBool "Should contain rc type" $
                    "reussir.rc" `isInfixOf` resultStr || "rc" `isInfixOf` resultStr
                assertBool "value should come before region" $
                    comesAfter "value(" "region(" resultStr
            ]
        , testGroup
            "RcFreeze"
            [ testCase "RcFreeze Codegen" $ do
                ...
                let resultStr = T.unpack result
                assertBool "Should contain reussir.rc.freeze" $
                    "reussir.rc.freeze" `isInfixOf` resultStr
                assertBool "Should contain result %2" $ "%2 = " `isInfixOf` resultStr
                assertBool "Should contain input %1" $ "%1" `isInfixOf` resultStr
                assertBool "Should contain rc type" $
                    "reussir.rc" `isInfixOf` resultStr || "rc" `isInfixOf` resultStr
                assertBool "Should contain ref type" $
                    "reussir.ref" `isInfixOf` resultStr || "ref" `isInfixOf` resultStr
                assertBool "Input should come in parentheses" $ "(%1" `isInfixOf` resultStr
                assertBool "Result should come before operation" $
                    comesAfter "%2 = " "reussir.rc.freeze" resultStr
            ]
        , testGroup
            "RcBorrow"
            [ testCase "RcBorrow Codegen" $ do
                ...
                let resultStr = T.unpack result
                assertBool "Should contain reussir.rc.borrow" $
                    "reussir.rc.borrow" `isInfixOf` resultStr
                assertBool "Should contain result %2" $ "%2 = " `isInfixOf` resultStr
                assertBool "Should contain input %1" $ "%1" `isInfixOf` resultStr
                assertBool "Should contain rc type" $
                    "reussir.rc" `isInfixOf` resultStr || "rc" `isInfixOf` resultStr
                ...
                assertBool "Input should come in parentheses" $ "(%1" `isInfixOf` resultStr
                assertBool "Result should come before operation" $
                    comesAfter "%2 = " "reussir.rc.borrow" resultStr
            ]
        , testGroup
            "RcIsUnique"
            [ testCase "RcIsUnique Codegen" $ do
                ...
                let resultStr = T.unpack result
                assertBool "Should contain reussir.rc.is_unique" $
                    "reussir.rc.is_unique" `isInfixOf` resultStr
                assertBool "Should contain result %2" $ "%2 = " `isInfixOf` resultStr
                assertBool "Should contain input %1" $ "%1" `isInfixOf` resultStr
                assertBool "Should contain rc type" $
                    "reussir.rc" `isInfixOf` resultStr || "rc" `isInfixOf` resultStr
                ...
                assertBool "Input should come in parentheses" $ "(%1" `isInfixOf` resultStr
                assertBool "Result should come before operation" $
                    comesAfter "%2 = " "reussir.rc.is_unique" resultStr
            ]
        , testGroup
            "CompoundCreate"
            [ testCase "CompoundCreate Codegen with single field" $ do
                ...
                let resultStr = T.unpack result
                assertBool "Should contain reussir.record.compound" $
                ...
                assertBool "Result should come before operation" $
                    comesAfter "%2 = " "reussir.record.compound" resultStr
            , testCase "CompoundCreate Codegen with multiple fields" $ do
                ...
                assertBool "Should contain all field values" $
                    "%1" `isInfixOf` resultStr
                        && "%2" `isInfixOf` resultStr
                        && "%3" `isInfixOf` resultStr
                assertBool "Should contain i32 type" $ "i32" `isInfixOf` resultStr
                assertBool "Should contain i64 type" $ "i64" `isInfixOf` resultStr
                assertBool "Should contain boolean type or i1" $
                    "i1" `isInfixOf` resultStr || "bool" `isInfixOf` resultStr
        ...
        , testGroup
            ...
            [ testCase "VariantCreate Codegen with tag 0" $ do
                ...
                let resultStr = T.unpack result
                assertBool "Should contain reussir.record.variant" $
                ...
                assertBool "Should contain i64 type" $ "i64" `isInfixOf` resultStr
                assertBool "Result should come before operation" $
                    comesAfter "%2 = " "reussir.record.variant" resultStr
        ...
        , testGroup
            ...
            [ testCase "VariantDispatch Codegen without result" $ do
                ...
                let resultStr = T.unpack result
                assertBool "Should contain reussir.record.dispatch" $
                    "reussir.record.dispatch" `isInfixOf` resultStr
                assertBool "Should contain input %1" $ "%1" `isInfixOf` resultStr
                assertBool "Should contain case tags" $
                    "[0, 1]" `isInfixOf` resultStr || "[0,1]" `isInfixOf` resultStr
                assertBool "Should contain func.return" $ "func.return" `isInfixOf` resultStr
                assertBool "Dispatch should come before cases" $
                    comesAfter "reussir.record.dispatch" "{" resultStr
                ...
                result <-
                    runCodegenForInstr
                        ( IR.VariantDispatch
                            (typedVal 1 primitiveI64)
                            ( IR.VariantDispData
                                [([42], blockWithArgs [] [IR.Return (Just (typedVal 2 primitiveI32))])]
                            )
                            (Just (typedVal 3 primitiveI32))
                        )
                let resultStr = T.unpack result
            ...
            , testCase "VariantDispatch Codegen with multiple cases" $ do
                ...
                assertBool "Should contain reussir.record.dispatch" $
                    "reussir.record.dispatch" `isInfixOf` resultStr
                assertBool "Should contain multiple case tags" $
                    ( "[0]" `isInfixOf` resultStr
                        || "[1, 2]" `isInfixOf` resultStr
                        || "[1,2]" `isInfixOf` resultStr
                    )
            ]
        , testGroup
            ...
            [ testCase "RefProject Codegen" $ do
                ...
                let resultStr = T.unpack result
                assertBool "Should contain reussir.ref.project" $
                    "reussir.ref.project" `isInfixOf` resultStr
                assertBool "Should contain result %2" $ "%2 = " `isInfixOf` resultStr
                assertBool "Should contain input %1" $ "%1" `isInfixOf` resultStr
                ...
                assertBool "Should contain ref type" $
                    "reussir.ref" `isInfixOf` resultStr || "ref" `isInfixOf` resultStr
                assertBool "Result should come before operation" $
                    comesAfter "%2 = " "reussir.ref.project" resultStr
        ...
        , testGroup
            ...
            [ testCase "RefSpill Codegen" $ do
                ...
                let resultStr = T.unpack result
                assertBool "Should contain reussir.ref.spill" $
                    "reussir.ref.spill" `isInfixOf` resultStr
                assertBool "Should contain result %2" $ "%2 = " `isInfixOf` resultStr
                assertBool "Should contain input %1" $ "%1" `isInfixOf` resultStr
                assertBool "Result should come before operation" $
                    comesAfter "%2 = " "reussir.ref.spill" resultStr
            ]
        , testGroup
            "RefLoad"
            [ testCase "RefLoad Codegen" $ do
                ...
                let resultStr = T.unpack result
                assertBool "Should contain reussir.ref.load" $
                    "reussir.ref.load" `isInfixOf` resultStr
                assertBool "Should contain result %2" $ "%2 = " `isInfixOf` resultStr
                assertBool "Should contain input %1" $ "%1" `isInfixOf` resultStr
                assertBool "Result should come before operation" $
                    comesAfter "%2 = " "reussir.ref.load" resultStr
            ]
        , testGroup
            "RefStore"
            [ testCase "RefStore Codegen" $ do
                ...
                let resultStr = T.unpack result
                assertBool "Should contain reussir.ref.store" $
                    "reussir.ref.store" `isInfixOf` resultStr
        ...
        , testGroup
            ...
            [ testCase "RegionRun Codegen without result" $ do
                ...
                let resultStr = T.unpack result
                assertBool "Should contain reussir.region.run" $
                    "reussir.region.run" `isInfixOf` resultStr
                assertBool "Should contain func.return" $ "func.return" `isInfixOf` resultStr
        ...
        , testGroup
            ...
            [ testCase "Yield Codegen with YieldClosure without result" $ do
                ...
                let resultStr = T.unpack result
                assertBool "Should contain reussir.closure.yield" $
                    "reussir.closure.yield" `isInfixOf` resultStr
            ...
            , testCase "Yield Codegen with YieldRegion without result" $ do
                ...
                let resultStr = T.unpack result
                assertBool "Should contain reussir.region.yield" $
                    "reussir.region.yield" `isInfixOf` resultStr
        ...
        , testGroup
            ...
            [ testCase "ClosureCreate Codegen" $ do
                ...
                result <-
                    runCodegenForInstr
                        ( IR.ClosureCreate
                            ( blockWithArgs
                                [typedVal 1 primitiveI32]
                                [IR.Return (Just (typedVal 2 primitiveI64))]
                            )
                            (typedVal 3 closureType)
                        )
                let resultStr = T.unpack result
                assertBool "Should contain reussir.closure.create" $
                    "reussir.closure.create" `isInfixOf` resultStr
                assertBool "Should contain result %3" $ "%3 = " `isInfixOf` resultStr
                ...
                assertBool "Result should come before operation" $
                    comesAfter "%3 = " "reussir.closure.create" resultStr
            ]
        , testGroup
            "ClosureApply"
            [ testCase "ClosureApply Codegen" $ do
                ...
                let resultStr = T.unpack result
                assertBool "Should contain reussir.closure.apply" $
                    "reussir.closure.apply" `isInfixOf` resultStr
                assertBool "Should contain result %3" $ "%3 = " `isInfixOf` resultStr
                assertBool "Should contain target %1" $ "%1" `isInfixOf` resultStr
                assertBool "Should contain argument %2" $ "%2" `isInfixOf` resultStr
                assertBool "Should contain 'to' keyword" $ " to " `isInfixOf` resultStr
                assertBool "Should contain type annotation" $ " : " `isInfixOf` resultStr
                assertBool "Argument should come before target ((%arg) to (%closure) format)" $
                    comesAfter "%2" "%1" resultStr
            ]
        , testGroup
            "ClosureEval"
            [ testCase "ClosureEval Codegen without result" $ do
                ...
                let resultStr = T.unpack result
                assertBool "Should contain reussir.closure.eval" $
                    "reussir.closure.eval" `isInfixOf` resultStr
                assertBool "Should contain target %1" $ "%1" `isInfixOf` resultStr
                assertBool "Should not contain result assignment" $
                    not ("%2 = " `isInfixOf` resultStr)
        ...
        , testGroup
            ...
            [ testCase "ClosureUniqify Codegen" $ do
                ...
                let resultStr = T.unpack result
                assertBool "Should contain reussir.closure.uniqify" $
                    "reussir.closure.uniqify" `isInfixOf` resultStr
                assertBool "Should contain result %2" $ "%2 = " `isInfixOf` resultStr
                assertBool "Should contain input %1" $ "%1" `isInfixOf` resultStr
                assertBool "Result should come before operation" $
                    comesAfter "%2 = " "reussir.closure.uniqify" resultStr
            ]
        , testGroup
            "IfThenElse"
