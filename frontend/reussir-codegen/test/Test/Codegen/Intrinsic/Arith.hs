...
runEmptyCodegenAsText codegen =
  let spec = C.TargetSpec "module" "output.mlir" B.OptDefault B.OutputObject B.LogInfo
   in do 
    ...
    pure $ TB.toLazyText (C.builder finalCtx)
...
primitiveI64 = TT.TypePrimitive (TT.PrimInt TT.PrimInt64)

primitiveF32 :: TT.Type
primitiveF32 = TT.TypePrimitive (TT.PrimFloat TT.PrimFloat32)

primitiveF64 :: TT.Type
primitiveF64 = TT.TypePrimitive (TT.PrimFloat TT.PrimFloat64)

primitiveF128 :: TT.Type
primitiveF128 = TT.TypePrimitive (TT.PrimFloat TT.PrimFloat128)
...
arithTests =
  testGroup
    ...
    [ testCase "ADDI Codegen" $ do
        text <- runCodegenForICall (addi32 3 1 2 $ I.IntOFFlag 0)
        text @?= "%3 = arith.addi %1, %2 : i32\n",
      testCase "ADDI Codegen with NSW" $ do
        text <- runCodegenForICall (addi32 3 1 2 $ I.IntOFFlag 1)
        text @?= "%3 = arith.addi %1, %2 overflow<nsw> : i32\n",
      testCase "ADDI Codegen with NSW NUW" $ do
        text <- runCodegenForICall (addi32 3 1 2 $ I.IntOFFlag 3)
        text @?= "%3 = arith.addi %1, %2 overflow<nsw nuw> : i32\n",
      testCase "ADDF Codegen" $ do
        text <- runCodegenForICall (addf64 3 1 2 $ I.FastMathFlag 0)
        text @?= "%3 = arith.addf %1, %2 : f64\n",
      testCase "ADDF Codegen with FastMath" $ do
        text <- runCodegenForICall (addf64 3 1 2 $ I.FastMathFlag 127)
        text @?= "%3 = arith.addf %1, %2 fastmath<fast> : f64\n",
      testCase "ADDF Codegen with Reassoc and NInf" $ do
        text <- runCodegenForICall (addf64 3 1 2 $ I.FastMathFlag 5)
        text @?= "%3 = arith.addf %1, %2 fastmath<reassoc ninf> : f64\n",
      testCase "CMPF Codegen oeq" $ do
        text <- runCodegenForICall (cmpf128 I.CFOeq (I.FastMathFlag 0) 1 2 3)
        text @?= "%3 = arith.cmpf oeq, %1, %2 : f128\n",
      testCase
        "Constant Codegen"
        text <- runCodegenForICall (constant "1.0" 0)
        text @?= "%0 = arith.constant 1.0 : f128\n",
