...
-- | REPL operation modes
data ReplMode
    = StmtMode  -- ^ Parse and add statements/definitions
    | EvalMode  -- ^ Parse and evaluate expressions
    deriving (Show, Eq)
...
data ResultKind
    ...
    | ResultBool
    | ResultUnit
    | ResultOther T.Text  -- ^ For non-primitive types, store type description
    deriving (Show, Eq)
...
data ReplState = ReplState
    ...
    , replCompiledFunctions :: H.CuckooHashTable T.Text ()
    ...
    , replMode :: ReplMode
    -- ^ Current REPL mode (statement or eval)
...
-- | Set the REPL mode
setReplMode :: ReplMode -> ReplState -> ReplState
setReplMode mode state = state { replMode = mode }

-- | Get the current REPL mode
getReplMode :: ReplState -> ReplMode
...
compileExpression ::
    ReplState ->
    T.Text ->        -- ^ Original source text (for error reporting)
    SynExpr.Expr ->  -- ^ Parsed expression
...
compileExpression state _sourceText expr = do
    ...
    B.withReussirLogger logLevel loggerName $ \logger -> do
        runEff $ L.runLog "REPL" logger (toEffLogLevel logLevel) $ runPrim $ do
            ...
            ((elaboratedExpr, exprType), finalSemiCtx) <- runState (replSemiContext state) $ do
            ...
            if translationHasFailed finalSemiCtx
                then do
                    ...
                    mlirResult <- generateExpressionModule 
                        exprFuncName 
                        elaboratedExpr 
                        exprType 
                        logLevel 
...
generateExpressionModule ::
    ...
    T.Text ->           -- ^ Function name for the wrapper
    Semi.Expr ->        -- ^ Elaborated expression
    Semi.Type ->        -- ^ Expression type
...
generateExpressionModule funcName semiExpr exprType logLevel state = do
    ...
    case tyResult of
        ...
        Right fullType -> do
            ...
            loweringCtx <- createLoweringContext
                repository
            ...
            -- Lower all functions (including the wrapper and any defined functions)
            allFuncs <- liftIO $ H.toList (ctxFunctions finalFullCtx)
            irModule <- runLoweringToModule loweringCtx $ do
                forM_ allFuncs $ \(_, func) -> lowerFunction func
...
typeToResultKind (Semi.TypeFP (IEEEFloat 16)) = ResultF16
