...
import Reussir.Core.Data.Semi.Context (SemiEff, SemiContext(..), LocalSemiContext(..), knownRecords)
import Reussir.Core.Semi.Context (emptySemiContext, emptyLocalSemiContext)
import Reussir.Core.Data.Semi.Record (Record(..), RecordFields(..), RecordKind(..))
import Reussir.Parser.Types.Stmt (Visibility(..))
import qualified Reussir.Parser.Types.Capability as Cap
import qualified Reussir.Parser.Types.Expr as Syn
import qualified Reussir.Core.Data.Semi.Type as SemiType
import qualified Data.Vector.Strict as V
import qualified Data.HashTable.IO as H
import qualified Effectful.State.Static.Local as State
import Effectful (runEff, liftIO, inject)
...
tests :: TestTree
tests = testGroup "Reussir.Core.Semi.PatternMatch"
    [ testCase "normalize named record" (testNormalizeNamed "testNormalizeNamed")
    , testCase "normalize named record (unknown field)" (testNormalizeNamedUnknown "testNormalizeNamedUnknown")
    , testCase "normalize named record (positional arg)" (testNormalizeNamedPositional "testNormalizeNamedPositional")
    , testCase "normalize named record (missing field)" (testNormalizeNamedMissing "testNormalizeNamedMissing")
    , testCase "normalize positional record" (testNormalizePositional "testNormalizePositional")
    , testCase "normalize positional record (too many)" (testNormalizePositionalTooMany "testNormalizePositionalTooMany")
    , testCase "normalize positional record (named arg)" (testNormalizePositionalNamed "testNormalizePositionalNamed")
    ]
...
testNormalizeNamedUnknown name = runSemi name $ do
    ...
    fieldsRef <- newIORef' Nothing
    span' <- State.gets currentSpan 
    let record = Record { recordName = recName, recordTyParams = [], recordFields = fieldsRef, recordKind = StructKind, recordVisibility = Public, recordDefaultCap = Cap.Value, recordSpan = span' }
    records <- State.gets knownRecords
    liftIO $ H.insert records recName record
...
testNormalizePositional name = runSemi name $ do
    ...
    -- Exact match
    let args1 = V.fromList [ Syn.PatternCtorArg Nothing Syn.WildcardPat, Syn.PatternCtorArg Nothing Syn.WildcardPat ]
    res1 <- normalizeCtorPattern recName args1 False
    liftIO $ assertBool "Should succeed" (isJust res1)
...
testNormalizePositionalTooMany name = runSemi name $ do
    ...
    let args = V.fromList [ Syn.PatternCtorArg Nothing Syn.WildcardPat, Syn.PatternCtorArg Nothing Syn.WildcardPat ]
    res <- normalizeCtorPattern recName args False
