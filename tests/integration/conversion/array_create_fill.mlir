...
// RUN: %reussir-opt %s --reussir-attach-native-target --reussir-token-instantiation --sccp --canonicalize --loop-invariant-code-motion --reussir-convert-to-std --reussir-acquire-drop-expansion | %FileCheck %s --implicit-check-not=reussir.array.fill_pattern
...
module {
  ...
  // CHECK: scf.for %[[I:.*]] =
  // CHECK: scf.for %[[J:.*]] =
  // CHECK: memref.store %arg0, %[[VIEW]][%[[I]], %[[J]]]
  // CHECK: return
  ...
  // CHECK: memref.store %arg1, %[[VIEW]][%[[I]], %[[J]]]
  // CHECK-NOT: reussir.rc.inc
  // CHECK: return
  ...
  // CHECK: %[[VALUE:.*]] = arith.constant 7 : index
  // CHECK: scf.for
